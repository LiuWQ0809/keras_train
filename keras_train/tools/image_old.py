...
class Iterator(object):
    ...
    def _flow_index(self, n, batch_size=32, shuffle=False, seed=None):
        ...
        while 1:
            ...
            # if n >= current_index + batch_size + self.excess_elements:
...
def process_image_pipeline_json(tup):
    ...
    if fstatus == "tumor":
        ...
        # 1. Load img and corresponding mask.
        ...
        if (width > target_size[0]) and (height > target_size[1]):
            for i in range(nb_crop):
                ...
                get_mask = mask_original.crop((get_x, get_y,
                                               get_x + target_size[0],
                                               get_y + target_size[1]))
                get_mask = img_to_array(get_mask, dim_ordering=dim_ordering)  # (299, 299, 3)
    ...
    # Type 2: Normal (tissue or background)
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator,
        ...
        for subdir in classes:
            ...
            # for roots,dirs,_ in _recursive_list(subpath):
            dirs = os.listdir(subpath)
            for dir in dirs:
                # print(dir)
                if dir=="patch_1":
                    file_path=os.path.join(subpath,dir)
                    file_paths.append(file_path)
                    print(file_path)
                else:
                    pass
        for file_path in file_paths:
            # for root, _, files in _recursive_list(file_path):
            # print(len(file_path))
            # a = time.time()
        ...
        print ("[INFO] Without class balance (old version).")
        print('[INFO] Originally found %d images belonging to %d classes.' % (self.nb_sample_init, self.nb_class))
        ...
        for subdir in classes:
            ...
            for dir in dirs:
                if dir=="patch_1":
                    ab_path=os.path.join(subpath,dir)
                else:
                    pass
            # for root, _, files in _recursive_list(ab_path):
            fanmes = os.listdir(ab_path)
            for fname in fanmes:
                is_valid = False
                for extension in white_list_formats:
                    if fname.lower().endswith('.' + extension):
                ...
                if is_valid:
                    self.classes[i] = self.class_indices[subdir]
                    i += 1
                    absolute_path = os.path.join(ab_path, fname)
                    self.filenames.append(os.path.relpath(absolute_path, directory))
                    self.filenames_relative_path.append(os.path.relpath(absolute_path, ab_path))

            # print("test lable")
            print(i)
            print(subdir)
            # print(self.classes[i-1])
            # print(self.classes[i-2])
            # if i==23133:
        ...
        if self.phase == "test":
            files_np = np.array(self.filenames_relative_path)
    ...
    def next(self):
        ...
        if self.pool:
            ...
            result = self.pool.map(process_image_pipeline_dir, ((pipeline,
                                                                 ...
                                                                 self.target_size,
                                                                 self.dim_ordering,
                                                                 self.rngs[i % self.batch_size]) for i, j in
                                                                enumerate(index_array)))
            batch_x = np.array(result)
        else:
...
class JsonIterator(Iterator):
    ...
    def __init__(self, json_file_path, image_data_generator,
                 ...
                 follow_links=False, pool=None, nb_gpu=4):
        if dim_ordering == 'default':
            dim_ordering = K.image_dim_ordering()
        ...
        self.patch_meta = []
        # Load all json files in the directory (either training or testing), sort
        # json_list = listdir(json_file_path)
        json_list=[]
        for rootss,dirss,_ in os.walk(json_file_path):
            for dirs in dirss:
                for roots, dir ,_ in os.walk(os.path.join(rootss,dirs)) :
                    for d in dir:
                        # print(os.path.join(roots,d))
                        for root,_,files, in os.walk(os.path.join(roots,d)):
                            for d in files:            
                                json_file=os.path.join(root,d)

                                json_list.append(json_file)
                                # print(json_file)
        json_list.sort()
       
        # Read json file and each dictionary
        # Append the dictionary into list according to the status (tumor or normal)
        print("[INFO] Loading and checking all Jsons (without class balancing)...")

        # nb_normal = 0
        # nb_tumor = 0

        # for json_file in json_list:
        #     current_json_path = json_file_path + '/' + json_file

        #     with open(current_json_path) as file:
        #         json_array = json.loads(file.read())

        #     for dicts in json_array:
        #         status = dicts["status"]
        ...
        #         self.patch_meta.append(dicts)
        for current_json_path in json_list:
            with open(current_json_path) as file:
                self.json_array=json.loads(file.read())


            if self.json_array["label"]=="tumor":
                patch_tumors=self.json_array["patches"]
                for i in range(0,len(patch_tumors)):
                    patch_tumor=patch_tumors[i]
                    patch_tumor["label"]="tumor"
                    self.patch_meta.append(patch_tumor)
        

            elif self.json_array["label"]=="lymphocyte":
                patch_lymphocytes=self.json_array["patches"]
                for j in range(0,len(patch_lymphocytes)):

                    patch_lymphocyte=patch_lymphocytes[j]
                     
                    patch_lymphocyte["label"]="lymphocyte"
                    self.patch_meta.append(patch_lymphocyte)

        # print("Shape of patch_meta:")
        # print(np.array(self.patch_meta).shape)

        self.nb_sample_init = len(self.patch_meta)

        # print("[!] Total number of tumor patches is %d" % (nb_tumor))
        # print("[!] Total number of normal patches is %d" % (nb_normal))
        print("[!] Original total number of patches is %d" % (self.nb_sample_init))

        # Modify the meta data to adapt with multi GPU training
        print("Number of GPU used for training: %d" % self.nb_gpu)
        multiple = self.nb_gpu * self.batch_size
        print("The multiple is: %d" % multiple)
        quotient = self.nb_sample_init//multiple
        print("Quotient: %d" % quotient)
        ...
        if nb_excess_patch == 0:
            ...
            for i in range(nb_excess_patch):
                self.patch_meta.pop(-1)
            print("[!] After pop the total number of patches is %d" % len(self.patch_meta))

        super(JsonIterator, self).__init__(self.nb_sample, batch_size, shuffle, seed)
    ...
    def next(self):
        ...
        if self.pool:
            ...
            pipeline = self.image_data_generator.pipeline()
            # map function: use the "process_image_pipeline_json" function
            # to process the second term
            results = self.pool.map(process_image_pipeline_json,
                                    ((pipeline,
                                    self.patch_meta[j],
                                    grayscale,
                                    self.target_size,
            ...
            # Check the shape the the results and concat
            result_np = np.asarray(results)
            nb_sample = result_np.shape[0]

            batch_x = np.asarray(result_np[:, 0])  # Shape: (nb_sample, 1)
            batch_y = np.asarray(result_np[:, 1])  # Shape: (nb_sample, 1)
